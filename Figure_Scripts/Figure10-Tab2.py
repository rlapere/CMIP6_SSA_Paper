...
fld = '/Users/rlapere/Desktop/CMIP_SSA/CMIP_DATA/rtmt/'

mods = ['a) IPSL-CM6','b) NorESM','c) UKESM']

ctrl = ['rtmt_Amon_IPSL-CM6A-LR-INCA_piClim-control_r1i1p1f1_gr_185001-187912.nc','rtmt_Amon_NorESM2-LM_piClim-control_r1i1p1f1_gn_000101-003012.nc','rtmt_Amon_UKESM1-0-LL_piClim-control_r1i1p1f4_gn_185001-189412.nc']
xss = ['rtmt_Amon_IPSL-CM6A-LR-INCA_piClim-2xss_r1i1p1f1_gr_185001-187912.nc','rtmt_Amon_NorESM2-LM_piClim-2xss_r1i1p1f1_gn_000101-003012.nc','rtmt_Amon_UKESM1-0-LL_piClim-2xss_r1i1p1f4_gn_185001-189412.nc']
...
for md in mods:
    ...
    rt2 = rtmt2.variables['rtmt'][:]
    rt = rtmt.variables['rtmt'][:]
    
    seas = range(len(rtmt2.variables['rtmt'][:,0,0]))
    ...
    print('Arctic DJF')
    print(str(np.mean(difarc_djf))+' +/- '+str(ciarc_djf))
    print(wilcoxon(difarc_djf))

    print('Antarctic DJF')
    print(str(np.mean(difant_djf))+' +/- '+str(ciant_djf))
    print(wilcoxon(difant_djf))  

    print('Arctic JJA')
    print(str(np.mean(difarc_jja))+' +/- '+str(ciarc_jja))
    print(wilcoxon(difarc_jja))

    print('Antarctic JJA')
    print(str(np.mean(difant_jja))+' +/- '+str(ciant_jja))
    print(wilcoxon(difant_jja))
    ...
    m3=axs[k].pcolormesh(lons,lats,diff_djf,cmap='ColdHot',vmin=-8,vmax=8,N=17)
    axs[k].plot(x,y,linewidth=0,marker='x',markersize=1,markerfacecolor=(0.25,0.25,0.25,0.75),markeredgecolor=(0.25,0.25,0.25,0.75))

    m=axs[k+3].pcolormesh(lons,lats,diff_jja,cmap='ColdHot',vmin=-8,vmax=8,N=17)
    ...
    m1=axs[k+6].pcolormesh(lons,lats,diff_jja,cmap='ColdHot',vmin=-8,vmax=8,N=17)
    ...
    m2=axs[k+9].pcolormesh(lons,lats,diff_djf,cmap='ColdHot',vmin=-8,vmax=8,N=17)
