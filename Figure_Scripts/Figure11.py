...
axs.format(suptitle='Historical and future trend in sea salt aerosol mass mixing ratio',coast=True, latlines=10,coastcolor='black',land=True,landcolor='white')
...
for md in mods:
    ...
    n85 = np.mean(var85[:,indn85,:],axis=(1,2))
    s85 = np.mean(var85[:,inds85,:],axis=(1,2))
    
    n26 = np.mean(var26[:,indn85,:],axis=(1,2))
    s26 = np.mean(var26[:,inds85,:],axis=(1,2))
    
    nhist = np.mean(varhist[:,indn,:],axis=(1,2))
    shist = np.mean(varhist[:,inds,:],axis=(1,2))
